...
LCG to klasyczny generator liniowy: \( x\_{n+1} = (a x_n + c) \bmod m \). Szybki i prosty, lecz podatny na korelacje oraz wrażliwy na dobór parametrów \(a, c, m\).
...
- Aktualizacja: \(x\_{n+1} = (a x_n + c) \bmod m\), gdzie \(a\) to mnożnik, \(c\) to przyrost, \(m\) to moduł.
